...
\olfileid[id]{nml}{prf}{prk}

\olsection{Pembuktian dalam \Log{K}}

Untuk berlatih melakukan pembuktian dalam sistem modal terkecil, kita
menunjukkan bahwa semua !!{formula}s valid pada sisi kiri
\olref[syn][sch]{tab:valid-invalidSchemas} dapat diberi pembuktian
dalam~\Log{K}.
...
\end{derivation}
Perhatikan bahwa !!{formula} pada baris~$9$ merupakan instansiasi tautologi
\[
...
\begin{proof}
  ...
  \end{derivation}
  !!^{formula}s pada baris $6$ dan~$9$ merupakan instansiasi tautologi
  \begin{align*}
...
\begin{proof}
  ...
  \end{derivation}
  !!^{formula}s pada baris $8$ dan~$10$ merupakan instansiasi tautologi
  \begin{align*}
  ...
  \end{derivation}
  Baris terakhir ialah $\lnot\Box p\lif\Diamond\lnot p$, sebab $\Diamond$
  didefinisikan sebagai $\lnot\Box\lnot$. !!^{formula} pada baris~$5$
  merupakan instansiasi tautologi
  \[
    (p \lif q) \lif (\lnot q \lif \lnot p).
  \]
  }}{% Box is defined
  ...
  \end{derivation}
  !!^{formula} pada baris~$4$ merupakan instansiasi tautologi
  \begin{align*}
...
\begin{prob}
  Temukan !!{derivation}s dalam~$\Log{K}$ bagi !!{formula}s berikut:
  \begin{enumerate}
