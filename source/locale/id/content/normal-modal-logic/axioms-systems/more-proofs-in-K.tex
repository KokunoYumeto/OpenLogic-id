...
\olfileid[id]{nml}{prf}{mpr}

\olsection{Pembuktian Lain dalam \Log{K}}

Mari kita lihat beberapa contoh lain !!{derivability} dalam~\Log{K}, kini
dengan menggunakan metode sederhana yang diperkenalkan dalam
\olref[der]{sec}.

\begin{prop}
  $\Log{K} \Proves \Box (!A \lif !B) \lif (\Diamond !A \lif \Diamond !B)$
\end{prop}
...
5. &  $\Log{K} \Proves \Box(!A \lif !B) \lif (\Diamond!A \lif \Diamond!B)$ & $\Diamond$ untuk $\lnot\Box\lnot$.
\end{derivation}
\end{proof}

\begin{prop}
$\Log{K}\Proves \Box!A \lif (\Diamond(!A \lif !B) \lif \Diamond !B)$
\end{prop}
...
\begin{proof}
  ...
  \Diamond !B)$ & $\Diamond$ untuk $\lnot\Box\lnot$.
  \end{derivation}
\end{proof}
...
\begin{proof}
  \begin{derivation}
    ...
    $\Diamond$ untuk $\lnot\Box\lnot$\\
    5. & $\Log{K} \Proves \Diamond!B \lif \Diamond(!A \lor !B)$ & dengan cara serupa\\
...
\begin{proof}
  \begin{derivation}
    ...
    \Diamond!B \lif \Diamond!A)$ & $\Diamond$ untuk $\lnot\Box\lnot$\\
    7. & $\Log{K} \Proves \Diamond(!A\lor!B) \lif (\Diamond!B \lor \Diamond!A)$ & \PL, 6. \\
  \end{derivation}
\end{proof}

\begin{prob}
  Tunjukkan bahwa klaim-klaim !!{derivability} berikut berlaku:
  \begin{enumerate}
